 ...
 * @since 20/02/2017
 */
public class AdvertingInteractorTest {

    AdvertingInteractor interactor;
    GumTreeApi api;
    SchedulerProvider scheduler;
    Advert expectedResult;
    ...
    @Before
    public void setup() {
        api = mock(GumTreeApi.class);
        scheduler = mock(SchedulerProvider.class);
        // Set up the stub we want to return in the mock
        Advert advertStub = new Gson().fromJson(MACHINE_ONE, Advert.class);
        ...
        when(api.getAdvert("1234-1234-1234-1234"))
                .thenReturn(Observable.just(expectedResult));
        // create a real new interactor using mocked api and scheduler
        interactor = new AdvertingInteractorImpl(api, scheduler);
    }

    @Test
    public void testGetAdvert() throws Exception {
        TestSubscriber<Advert> testSubscriber = new TestSubscriber<>();

        // call interactor with some random params
        interactor.loadAdvert("1234-1234-1234-1234")
